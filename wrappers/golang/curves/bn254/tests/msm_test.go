...
import (
	...
	"github.com/ingonyama-zk/icicle-gnark/v3/wrappers/golang/curves/bn254/msm"
...
func testMSM(suite *suite.Suite) {
	...
	for _, power := range []int{2, 3, 4, 5, 6} {
		...
		var pMain icicleBn254.Projective
		...
		suite.Equal(e, runtime.Success, "Msm failed")
		outHostMain := make(core.HostSlice[icicleBn254.Projective], 1)
...
func testMSMBatch(suite *suite.Suite) {
	...
	for _, power := range []int{5, 6} {
		for _, batchSize := range []int{1, 3, 5} {
			...
			outHostMain := make(core.HostSlice[icicleBn254.Projective], batchSize)
...
//	wg.Add(2)
//
//	outHostMain1 := make(core.HostSlice[icicleBn254.Projective], 1)
//	outHostMain2 := make(core.HostSlice[icicleBn254.Projective], 1)
//	// Cuda run
