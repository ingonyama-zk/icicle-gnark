...
import (
	...
	icicleBls12_377 "github.com/ingonyama-zk/icicle-gnark/v3/wrappers/golang/curves/bls12377"
	"github.com/ingonyama-zk/icicle-gnark/v3/wrappers/golang/curves/bls12377/msm"
...
func testMSM(suite *suite.Suite) {
	...
	for _, power := range []int{2, 3, 4, 5, 6} {
		...
		var pMain icicleBls12_377.Projective
		...
		suite.Equal(e, runtime.Success, "Msm failed")
		outHostMain := make(core.HostSlice[icicleBls12_377.Projective], 1)
...
func testMSMBatch(suite *suite.Suite) {
	...
	for _, power := range []int{5, 6} {
		for _, batchSize := range []int{1, 3, 5} {
			...
			outHostMain := make(core.HostSlice[icicleBls12_377.Projective], batchSize)
...
//	wg.Add(2)
//
//	outHostMain1 := make(core.HostSlice[icicleBls12_377.Projective], 1)
//	outHostMain2 := make(core.HostSlice[icicleBls12_377.Projective], 1)
//	// Cuda run
...
//
//	outHostMain := make(core.HostSlice[icicleBls12_377.Projective], 1)
//	var one icicleBls12_377.ScalarField
//	one.One()
//	ones := []icicleBls12_377.ScalarField{one, one}
//	e = msm.Msm(
